// cppreference link https://en.cppreference.com/w/cpp/algorithm/lexicographical_compare_three_way
#include <algorithm>
#include <cctype>
#include <compare>
#include <iomanip>
...
void show_result(std::string_view s1, std::string_view s2, std::strong_ordering o)
{
    ...
    (o < 0) ? std::cout << "less than " : (o > 0) ? std::cout << "greater than "
                                                  : std::cout << "equal to ";
    std::cout << quoted(s2) << '\n';
}
...
int main()
{
    auto cmp_icase = [](char x, char y) {
        ...
        return (ux < uy) ? std::strong_ordering::less : (ux > uy) ? std::strong_ordering::greater
