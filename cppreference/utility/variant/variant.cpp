// cppreference link https://en.cppreference.com/w/cpp/utility/variant/variant
#include <cassert>
#include <iostream>
#include <string>
#include <variant>
...
template <class Os>
Os& operator<<(Os& os, const std::vector<int>& v)
{
    os << "{ ";
    for (int e : v) {
        std::cout << e << ' ';
...
{
    {
        ...
        assert(std::holds_alternative<int>(var) && var.index() == 0 && std::get<int>(var) == 0);
    }
    {
