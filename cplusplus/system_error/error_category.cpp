// cplusplus link https://cplusplus.com/reference/system_error\error_category
...
enum class custom_errc { success = 0,
    client_error,
    server_error,
    other };
namespace std {
template <>
struct is_error_condition_enum<custom_errc> : public true_type { };
}
...
public:
    ...
    {
        if ((ev >= 200) && (ev < 300))
            return std::error_condition(custom_errc::success);
        else if ((ev >= 400) && (ev < 500))
            return std::error_condition(custom_errc::client_error);
        else if ((ev >= 500) && (ev < 600))
            return std::error_condition(custom_errc::server_error);
        else
...
int main()
{
    ...
    if (e == custom_errc::success)
        std::cout << "Success: " << e.message();
    else if (e == custom_errc::client_error)
        std::cout << "Client Error: " << e.message();
    else if (e == custom_errc::server_error)
